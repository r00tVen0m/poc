        ...
        foreach ($Attachment in $Item.Attachments) {
            ...
            if ($Attachment -is [Microsoft.Exchange.WebServices.Data.FileAttachment]) {
                ...
                try {
                    ...
                    # ==========================================
                    # SOLUTION 1: Save locally first, then copy
                    # ==========================================
                    $LocalPath = Join-Path $LocalTempFolder $CleanName
                    
                    # Save to local temp folder first (faster and avoids UNC/PSDrive issues)
                    [System.IO.File]::WriteAllBytes($LocalPath, $Attachment.Content)
                    Write-Host "[+] Saved locally: $LocalPath" -ForegroundColor Green

                    # Then copy to network share using PowerShell copy
                    $NetworkPath = Join-Path $DownloadFolder $CleanName
                    ...
                    # ==========================================
                    # SOLUTION 2 (Alternative - uncomment to use):
                    # Write directly to network using Set-Content
                    # ==========================================
                    # $NetworkPath = Join-Path $DownloadFolder $CleanName
                    # $Attachment.Content | Set-Content -Path $NetworkPath -Encoding Byte -Force
                    # Write-Host "[+] Saved directly: $NetworkPath" -ForegroundColor Green
